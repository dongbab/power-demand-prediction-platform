"""
Advanced Prediction Engine with Multiple Statistical Models
다양한 통계적 추론과 극값 추정 모델을 사용한 고급 예측 엔진
"""
...
class EnsemblePrediction:
    """앙상블 모델의 최종 예측 결과"""
    final_prediction: int  # 1kW 단위로 반올림된 최종 예측값
...
class AdvancedPredictionEngine:
    ...
    def predict_contract_power(self, data: pd.DataFrame, station_id: str, charger_type: str = None) -> EnsemblePrediction:
        """
        다양한 통계 모델을 사용하여 권고 계약 전력 예측
        Args:
        ...
        try:
            # 1. 극값 이론 모델들
            evt_models = self._extreme_value_models(power_data)
            model_predictions.extend(evt_models)
            
            # 2. 통계적 추론 모델들
            stat_models = self._statistical_inference_models(power_data)
            model_predictions.extend(stat_models)
            
            # 3. 시계열 기반 모델들
            ts_models = self._time_series_models(data)
            model_predictions.extend(ts_models)
            
            # 4. 머신러닝 기반 모델들
            ml_models = self._machine_learning_models(power_data)
            model_predictions.extend(ml_models)
            
        except Exception as e:
            self.logger.error(f"Model prediction failed for {station_id}: {e}")
            return self._fallback_prediction(station_id)
        
        # 앙상블 예측 수행
        return self._ensemble_prediction(model_predictions, power_data, station_id)
    ...
    def _preprocess_data(self, data: pd.DataFrame) -> np.ndarray:
        ...
        # 0보다 크고 200kW 이하의 데이터만 사용 (이상값 제거)
        ...
        power_data = power_data[(power_data >= max(0, lower_bound)) & (power_data <= upper_bound)]
        
        return power_data.values
    
    def _extreme_value_models(self, power_data: np.ndarray) -> List[ModelPrediction]:
        """극값 이론 기반 모델들"""
        models = []
        ...
        try:
            ...
            gev_ci = genextreme.interval(0.9, *gev_params)
            
            models.append(ModelPrediction(
            ...
            gumbel_ci = gumbel_r.interval(0.9, *gumbel_params)
    ...
    def _statistical_inference_models(self, power_data: np.ndarray) -> List[ModelPrediction]:
        """통계적 추론 기반 모델들"""
        models = []
        
        try:
            # 1. Bayesian Estimation
            bayesian_result = self._bayesian_estimation(power_data)
            if bayesian_result:
                models.append(bayesian_result)
            
            # 2. Bootstrap Confidence Interval
            bootstrap_result = self._bootstrap_method(power_data)
            if bootstrap_result:
                models.append(bootstrap_result)
            
            # 3. Quantile Regression
            quantile_result = self._quantile_regression(power_data)
            if quantile_result:
                models.append(quantile_result)
            
            # 4. Kernel Density Estimation
            kde_result = self._kernel_density_estimation(power_data)
            if kde_result:
                models.append(kde_result)
    ...
    def _time_series_models(self, data: pd.DataFrame) -> List[ModelPrediction]:
        ...
        try:
            if '충전시작일시' in data.columns:
                # 월별 최대값 추출
                data_copy = data.copy()
                data_copy['충전시작일시'] = pd.to_datetime(data_copy['충전시작일시'], errors='coerce')
                data_copy = data_copy.dropna(subset=['충전시작일시', '순간최고전력'])
                
                # 월별 집계
                monthly_max = data_copy.groupby(data_copy['충전시작일시'].dt.to_period('M'))['순간최고전력'].max()
    ...
    def _machine_learning_models(self, power_data: np.ndarray) -> List[ModelPrediction]:
        """머신러닝 기반 모델들"""
        models = []
        
        try:
            # 1. Ensemble Percentile Method
            percentiles = [90, 95, 98, 99]
            percentile_predictions = []
            
            for p in percentiles:
                pred = np.percentile(power_data, p)
                percentile_predictions.append(pred)
            ...
            models.append(ModelPrediction(
                ...
                confidence_interval=(np.percentile(power_data, 85), np.percentile(power_data, 99)),
                confidence_score=0.75,
                method_details={
                    "method": "Weighted Percentile Ensemble",
                    "percentiles": percentiles,
                    "weights": weights.tolist(),
                    "description": "다중 분위수의 가중 평균을 사용한 예측"
                }
            ))
            
            # 2. Robust Statistics
            robust_result = self._robust_statistics_method(power_data)
            if robust_result:
    ...
    def _bayesian_estimation(self, power_data: np.ndarray) -> Optional[ModelPrediction]:
        """베이지안 추정"""
        try:
            # 단순 베이지안 추정 (정규 분포 가정)
            n = len(power_data)
            sample_mean = np.mean(power_data)
            sample_std = np.std(power_data, ddof=1)
            ...
            posterior_mean = (prior_mean/(prior_std**2) + n*sample_mean/(sample_std**2)) * posterior_var
    ...
    def _bootstrap_method(self, power_data: np.ndarray, n_bootstrap: int = 1000) -> Optional[ModelPrediction]:
        """부트스트랩 방법"""
        try:
            ...
            for _ in range(n_bootstrap):
                # 복원 추출
                bootstrap_sample = np.random.choice(power_data, size=len(power_data), replace=True)
                # 95% 분위수 계산
                bootstrap_predictions.append(np.percentile(bootstrap_sample, 95))
            
            # 부트스트랩 예측값들의 평균
            prediction = np.mean(bootstrap_predictions)
            
            # 신뢰구간 (부트스트랩 분위수 방법)
            ci_lower = np.percentile(bootstrap_predictions, 5)
            ci_upper = np.percentile(bootstrap_predictions, 95)
            ...
            return ModelPrediction(
                ...
                method_details={
                    ...
                    "bootstrap_std": np.std(bootstrap_predictions),
    ...
    def _quantile_regression(self, power_data: np.ndarray) -> Optional[ModelPrediction]:
        ...
        try:
            ...
            if result.success:
                ...
                bootstrap_preds = []
                for _ in range(100):
                    indices = np.random.choice(len(power_data), len(power_data), replace=True)
                    boot_x = x[indices]
                    boot_y = y[indices]
                    boot_result = minimize(
                        lambda theta: np.sum(np.maximum(0.95 * (boot_y - (theta[0] + theta[1] * boot_x)), 
                    ...
                    if boot_result.success:
                        ...
                        bootstrap_preds.append(boot_intercept + boot_slope * len(power_data))
    ...
    def _kernel_density_estimation(self, power_data: np.ndarray) -> Optional[ModelPrediction]:
        """커널 밀도 추정"""
        try:
    ...
    def _exponential_smoothing(self, monthly_data: np.ndarray) -> Optional[ModelPrediction]:
        """지수 평활법"""
        try:
    ...
    def _linear_trend_analysis(self, monthly_data: np.ndarray) -> Optional[ModelPrediction]:
        """선형 추세 분석"""
        try:
            if len(monthly_data) < 3:
                return None
    ...
    def _robust_statistics_method(self, power_data: np.ndarray) -> Optional[ModelPrediction]:
        """강건 통계량 기반 방법"""
        try:
            # 중앙값과 MAD (Median Absolute Deviation) 사용
    ...
    def _ensemble_prediction(self, model_predictions: List[ModelPrediction], 
                           power_data: np.ndarray, station_id: str) -> EnsemblePrediction:
        """앙상블 예측 수행"""
        ...
        # 가중 평균 계산
        weighted_sum = sum(pred.predicted_value * weights[pred.model_name] 
                          for pred in model_predictions)
        
        # 충전기 타입별 최대 전력 제한 적용
        max_power = self.charger_limits.get(charger_type, 100) if charger_type else 100
        
        # 최대 전력 제한 및 1kW 단위로 반올림
        final_prediction = min(max_power, max(1, round(weighted_sum)))
        
        # 불확실성 계산 (예측값들의 표준편차)
        predictions_array = np.array([pred.predicted_value for pred in model_predictions])
        uncertainty = np.std(predictions_array)
        
        # 시각화 데이터 준비
        viz_data = self._prepare_visualization_data(model_predictions, power_data, final_prediction)
    ...
    def _prepare_visualization_data(self, model_predictions: List[ModelPrediction], 
                                  power_data: np.ndarray, final_prediction: int) -> Dict[str, Any]:
        """시각화용 데이터 준비"""
        
        # 히스토그램 데이터
        hist_counts, hist_bins = np.histogram(power_data, bins=30)
    ...
    def _fallback_prediction(self, station_id: str) -> EnsemblePrediction:
        ...
        return EnsemblePrediction(
            ...
            visualization_data={
                ...
                "models": [{"name": "Fallback_Conservative", "prediction": 50.0, 
                          "confidence_interval": (40.0, 60.0), "confidence_score": 0.6}],
                "statistics": {},
                "final_prediction": 50,
                "data_size": 0
            }
